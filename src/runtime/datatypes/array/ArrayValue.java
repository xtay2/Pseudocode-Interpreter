...
public final class ArrayValue extends Value implements Iterable<Value> {
	 ...
	 */
	private static Value[] cast(Value[] vals, DataType target) {
		...
		for (int i = 0; i < vals.length; i++) {
			if (!target.allowsNull && vals[i] == NULL)
				throw new CastingException("You cannot cast an array, that contains null values, to one, that doesn't allow them.");
	...
	public TextValue asText() {
		...
		for (int i = 0; i < length(); i++) {
			b.append(get(i).as(TypeConstants.TEXT).raw());
			if (i == length() - 1)
	 ...
	 */
	public Value set(Value val, String varName, int orgLine, ValueHolder... idxs) {
		...
		if (!val.matches(expType))
			throw new UnexpectedDataType(orgLine,
					"Tried to set the value " + val + " into \"" + varName + "\" when only values of type \"" + rules + "\" are allowed.");

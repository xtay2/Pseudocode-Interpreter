...
public abstract class MainExpression extends Expression {
	...
	public final void setScope(Scope s) {
		assert s != null : getDataPath() + ": Scope cannot be set to null.";
		if (myScope != null && myScope != s) {
			throw new AssertionError(getDataPath() + ": Tried to set the Scope of " + toString() + " to " + s.getScopeName()
					+ " but it was already set to " + myScope.getScopeName());
		}
		myScope = s;
	}
